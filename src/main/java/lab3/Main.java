...
public class Main {
    public static void main(String[] args) {
        ...
        SchoolService1 schoolService = new SchoolService1(students);

        // Test sorting by name
        System.out.println("Sorted by name:");
        schoolService.sortStudentsByName().forEach(student -> System.out.println(student.getName()));

        // Test sorting by date of birth
        System.out.println("\nSorted by date of birth:");
        schoolService.sortStudentsByDateOfBirth().forEach(student -> System.out.println(student.getName()));

        // Test sorting by average grade
        System.out.println("\nSorted by average grade:");
        schoolService.sortStudentsByAverageGrade().forEach(student ->
                System.out.println(student.getName() + " - " + student.getAverageGrade()));

        // Test filtering by part of the name
        System.out.println("\nFiltered by part of the name:");
        schoolService.getByPartName("Jo").forEach(student -> System.out.println(student.getName()));

        // Test filtering by current month
        System.out.println("\nFiltered by current month:");
        schoolService.getByCurrentMonth().forEach(student -> System.out.println(student.getName()));
